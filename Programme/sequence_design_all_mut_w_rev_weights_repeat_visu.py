...
def main(command_line_arguments=None):
    ...
    energy = Energy(sequence.seq_predictable, model, y_target, int(args.length), int(args.repeat)) 
    ...
    #line_t, = ax_ATGC.plot(range(counter+1), T_content, 'tab:darkgreen', label='T')
    ...
    with open(os.path.join(os.path.dirname(__file__),
        ...
        f_head.write("length\trepeat\tstep\tmodel\ttemperature\tdirectory\tweight_GC\tweight_seq\tweight_rev_comp\tweight_mut\ttarget_amplitude\ttarget_background\n")
        ...
        f_head.write('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' % (len_w,rep_w,stp_w,mod_w,temp_w,dir_w,GC_w,seq_w,rev_w,mut_w,amp_w,back_w))        
    
    dir_plot = str(args.directory)+'_plot'

    for i in range(int(args.steps)):
        accepted = step(sequence, energy, float(args.temperature), count_mutation, args.weight)
        ...
        MC_to_plot_full = sequence.GC_energy + energy.reg_energy_profile + energy.rev_energy_profile
        ...
        if i % 1 == 0: #i % 2 == 0:
            ...
            ax_energy.set_xlim(0, int(args.steps))#1.8*i)
