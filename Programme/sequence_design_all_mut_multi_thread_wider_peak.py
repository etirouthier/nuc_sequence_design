...
class Sequence:
    ...
    def _process(self, nucleotid):
        ...
        x_r = x_r.reshape(3*self.length,nw_seq_length,4)
...
#a virer
    ...
    def _get_one_energy(self, y): #, seq):
        ...
        #atgc = seq._get_atgc_content_mutated(seq.sequence)
        #GC = atgc[2,:] + atgc[3,:]
        return 5*np.mean(np.abs(y - self.y_target)) - \
               self._get_corr(y) + 1# + sqrt((GC-0.4) * (GC-0.4)) #added weight on mae
...
def step(sequence, energy, temp):
    ...
    GC_energy_1D= GC_energy.reshape((3*sequence.length,))
    #print('GC_energy_1D:',GC_energy_1D.shape)
    #print('energy:',energy.energy_profile.shape)
    modified_energy = energy.energy_profile + GC_energy_1D
    #print('modified_energy:',modified_energy.shape)
    esum = np.sum(np.exp(-(modified_energy)/temp))
    #print('esum:',esum)
    ...
    while eless <= rand:
        ...
        eless = eless + exp(-(modified_energy[index])/temp)
        index = index + 1
...
def main(command_line_arguments=None):
    ...
    shift = 0.3 #0.2
    #len601 = 147
    len_wide = int(args.length)
    #x_gauss = np.arange(1,len601,1)
    x_gauss = np.arange(1,len_wide,1)
    #y_target_ = shift + \
    #    amp*np.exp(-((x_gauss - ((len601-1)/2.))**2/(2.*len601*len601/16.)))
        
    y_target_ = shift + \
        amp*np.exp(-((x_gauss - ((len_wide-1)/2.))**2/(2.*len_wide*len_wide/16.)))
